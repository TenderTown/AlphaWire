...
export const WORDS_MOST = [
  ...
  { text: "Contrarian", dims: { AA: 3 } }, { text: "Agreeable", dims: { AA: -1, SO: 1 } },
  ...
  { text: "Intuitive", dims: { ED: -1, EV: 1 } }, { text: "Pragmatic", dims: { AQ: 1, ED: 1 } },
];
...
export const BEHAVIORAL_QS = [
  ...
  { q: "If I tell myself I'm going to do something (exercise, diet, routine), I:", opts: [
    { t: "Start strong but rarely sustain it", p: { ED: 2 } },
    { t: "Stick with it for a while but eventually drift", p: { ED: 4 } },
    { t: "Maintain it consistently with occasional breaks", p: { ED: 7 } },
    { t: "Follow through with near-mechanical consistency once committed", p: { ED: 10 } },
  ]},
  ...
  { q: "I would describe my relationship with authority figures as:", opts: [
    ...
    { t: "Questioning — I challenge authority regularly", p: { AA: 10 } },
  ]},
  ...
  { q: "My interests and hobbies over the past 5 years have been:", opts: [
    { t: "Consistent — I've deepened my mastery in the same areas", p: { AQ: 1 } },
    { t: "Mostly the same with a few additions", p: { AQ: 4 } },
    { t: "A mix of old favorites and new explorations", p: { AQ: 6 } },
    { t: "Constantly evolving — I'm always discovering new things", p: { AQ: 10 } },
  ]},
  ...
  { q: "By the end of a long, mentally demanding day, I typically:", opts: [
    { t: "Am completely wiped — can barely think straight", p: { TS: 1 } },
    { t: "Am tired but functional — need a quiet evening", p: { TS: 4 } },
    { t: "Still have some gas in the tank — could do more if needed", p: { TS: 7 } },
    { t: "Feel like I'm just hitting my stride — I could keep going", p: { TS: 10 } },
  ]},
  ...
  { q: "When it comes to sustained focus on a complex task, I can maintain peak concentration for:", opts: [
    { t: "30-60 minutes before I need a break", p: { TS: 2 } },
    { t: "1-2 hours comfortably", p: { TS: 4 } },
    { t: "3-4 hours before fatigue sets in", p: { TS: 7 } },
    { t: "5+ hours — deep focus is my natural state", p: { TS: 10 } },
  ]},
];
...
export function calculateScores({ wordsSelected, wordsLeast, behAns, pairAns, specVals, idealVals }) {
  ...
  wordsSelected.forEach(idx => {
    ...
    Object.entries(w.dims).forEach(([k, v]) => { raw[k] += v; counts[k]++; });
  });
  ...
  Object.entries(pairAns).forEach(([qi, side]) => {
    ...
    Object.entries(choice.dims).forEach(([k, v]) => { raw[k] += v; counts[k]++; });
  });

  Object.entries(specVals).forEach(([dim, val]) => {
    raw[dim] += Number(val) * 0.5;
    counts[dim]++;
  });
  ...
  DIMS.forEach(d => {
    ...
    selfScores[d.key] = Math.max(1, Math.min(10, base + 5));
  });
  ...
  DIMS.forEach(d => {
    ...
    const gap = ideal - self;
    final[d.key] = Math.max(1, Math.min(10, self + gap * 0.15));
  });
